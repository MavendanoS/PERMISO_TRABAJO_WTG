...
// .wrangler/tmp/bundle-silQhF/checked-fetch.js
...
function getWebAppScript() {
  return `
    ...
    if (newPassword.length < 8) {
        errorDiv.textContent = 'La contrase\xF1a debe tener al menos 8 caracteres';
        errorDiv.style.display = 'block';
        return;
    }
    ...
    window.openAprobarCierreModal = async function(permisoId, numeroPT) {
        const decision = confirm('\xBFDesea APROBAR el cierre de este permiso?
...
var AuthService = class {
  ...
  async verifyToken(token) {
    ...
    if (!h || !p || !s) throw new errors_default("Token inv\xE1lido", 401);
    const unsigned = `${h}.${p}`;
    const key = await crypto.subtle.importKey("raw", enc.encode(this.SECRET), { name: "HMAC", hash: "SHA-256" }, false, ["sign"]);
    const sigBuf = await crypto.subtle.sign("HMAC", key, enc.encode(unsigned));
    const expected = b64url(btoa(String.fromCharCode(...new Uint8Array(sigBuf))));
    if (s !== expected) throw new errors_default("Token inv\xE1lido", 401);
...
var InputSanitizer = class _InputSanitizer {
  static {
    __name(this, "InputSanitizer");
  }
  ...
  static sanitizeObject(obj) {
    ...
    for (const key of Object.keys(obj)) {
      sanitized[key] = _InputSanitizer.sanitizeString(obj[key]);
    }
    return sanitized;
  }
...
async function handleChangePassword(request, corsHeaders, env, services) {
  ...
  try {
    ...
    if (!newPassword || newPassword.length < 8) {
      return new Response(JSON.stringify({
        success: false,
        error: "La contrase\xF1a debe tener al menos 8 caracteres"
      }), {
        status: 400,
        headers: { "Content-Type": "application/json", ...corsHeaders }
      });
...
// .wrangler/tmp/bundle-silQhF/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-silQhF/middleware-loader.entry.ts
